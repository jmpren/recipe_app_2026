...
-- Households: collaborative meal planning (Phase 3). Members share a week plan
-- (meal_plan_entries.household_id), propose recipes per week, and upvote.
...
-- household_id on meal_plan_entries: null = personal, set = that household's plan
...
-- accepted friend's recipes, plus their households' shared data (Phase 3).
...
-- Households. is_household_member / is_household_owner are security definer to
-- break the recursive RLS on household_members; they leak only a boolean.
create or replace function public.is_household_member(hh uuid, uid uuid)
returns boolean language sql stable security definer set search_path = public, pg_temp as $$
  select exists (select 1 from public.household_members where household_id = hh and user_id = uid);
$$;
...
create policy "household plan: add" on meal_plan_entries for insert
  with check (household_id is not null and user_id = auth.uid()
...
create policy "read household votes" on proposal_votes for select
  using (exists (select 1 from public.meal_proposals p
...
-- ---------------------------------------------------------------------------
-- RPCs: plan_meal + household collaboration (create_household,
-- add_household_member, propose_meal, schedule_proposal). plan_meal takes an
-- optional household_id (null = personal). See docs/DATA_MODEL.md,
-- docs/API_CONTRACT.md and the …_households.sql migration for full bodies.
-- ---------------------------------------------------------------------------
create or replace function public.plan_meal(
  recipe_id uuid, planned_on date, slot text default 'dinner', household_id uuid default null
)
...
begin
  ...
  values (
    ...
    coalesce((
      ...
      where m.planned_on = plan_meal.planned_on and m.slot = v_slot
...
begin
  if v_user is null then raise exception 'create_household: not authenticated' using errcode = '28000'; end if;
  if v_name is null then raise exception 'create_household: name is required' using errcode = '23514'; end if;
...
declare
  ...
  v_entry public.meal_plan_entries;
begin
  if v_user is null then raise exception 'schedule_proposal: not authenticated' using errcode = '28000'; end if;
  if v_slot not in ('breakfast', 'lunch', 'dinner', 'snack') then
...
-- ---------------------------------------------------------------------------
-- RPC: add_recipe_tag(recipe_id uuid, tag_name text) -> tags
-- Find-or-create a lowercased tag and link it to the recipe (Phase 2).
-- security invoker; the recipe_tags policy authorises the link only for the
-- owner. Not versioned. Removing / listing tags are plain table ops. See
-- docs/API_CONTRACT.md and the …_add_recipe_tag_rpc.sql migration.
-- ---------------------------------------------------------------------------
